...
function normalizeGeneratedModel(value) {
  ...
  const cleanCondition = (condition) => {
    ...
    return {
      ...
      // every other indicator. Falls back to value*2/3 when the AI didn't give a specific number
...
function buildPromptGuideLines(schema) {
  return [
    ...
    "drawdownFromWaveHigh 条件有一个专属字段 condition.waveThreshold（不是模型全局共用的，是这一条 condition 自己的）——价格从候选高点回落超过这个百分比，才会把该高点确认锁定为“波浪高点”，用来过滤噪音、只认真正的转折点；一个模型里如果有多条 drawdownFromWaveHigh 条件，各自可以有不同的确认阈值，不用互相牵制。默认规则：用户没有明确指定这个阈值时，取这条条件自己 value（回撤目标）的 2/3 作为 waveThreshold；取值必须大于等于1、且必须小于这条条件自己的 value——绝对不要生成接近0（比如0.1、0.5）或者大于等于 value 的数值：太小会导致“波浪高点”被任何一天的正常波动噪音刷新，起不到过滤转折点的作用；大于等于 value 则会导致高点要等回撤已经达到甚至超过要筛选的目标才被确认，参考的高点会滞后、失真。其他所有指标不需要这个字段，必须设为 null。",
