...
const BLOCK_RULE_ACTION_TYPES = ["targetPercent", "targetShares", "reducePercent", "addPercent", "exitAll"];
...
function normalizeGeneratedModel(value) {
  ...
  const cleanAction = (action) => {
    ...
    if (action.type === "addPercent") return { type: action.type, value: clamp(value, 0, 100, 0) };
  ...
  return {
    ...
    peVolumeRule: model.peVolumeRule && typeof model.peVolumeRule === "object" ? {
      ...
      highPePercentile: clamp(model.peVolumeRule.highPePercentile, 0, 100, 80),
      volumeMaDays: clamp(model.peVolumeRule.volumeMaDays, 2, 250, 20),
      volumeBuyMultiplier: clamp(model.peVolumeRule.volumeBuyMultiplier, 0.1, 10, 1.2),
      volumeSellMultiplier: clamp(model.peVolumeRule.volumeSellMultiplier, 0.1, 10, 0.7),
      lowPeTarget: clamp(model.peVolumeRule.lowPeTarget, 0, 100, 80),
      neutralTarget: clamp(model.peVolumeRule.neutralTarget, 0, 100, 40),
      highPeTarget: clamp(model.peVolumeRule.highPeTarget, 0, 100, 0),
    } : null,
  ...
  return {
    ...
    properties: {
      ...
      peVolumeRule: {
        ...
        additionalProperties: false,
        properties: {
          peLookbackDays: { type: "number" },
          lowPePercentile: { type: "number" },
          highPePercentile: { type: "number" },
          volumeMaDays: { type: "number" },
          volumeBuyMultiplier: { type: "number" },
          volumeSellMultiplier: { type: "number" },
          lowPeTarget: { type: "number" },
          neutralTarget: { type: "number" },
          highPeTarget: { type: "number" },
        },
      },
...
function buildPromptGuideLines(schema) {
  return [
    ...
    "block.action.type 只能是这5种：targetPercent(绝对目标仓位%——“调仓到90%”“仓位调整为100%”这类不管当前仓位是多少、直接定死到某个百分比的说法用这个)、targetShares(绝对目标股数——“买入1000股”“持有2000股”这类按股数说的用这个)、reducePercent(相对减仓——“减仓30%”“在当前仓位基础上减少20个百分点”这类说的是在当前仓位百分比上往下减一个幅度、不是减到某个绝对值的用这个，value填要减少的百分点数)、addPercent(相对加仓——跟 reducePercent 完全对称，“加仓20%”“增加仓位20%”“在当前基础上加20个百分点”这类说的是在当前仓位百分比上往上加一个幅度、不是加到某个绝对值的用这个，value填要增加的百分点数；如果没有现金买不了，这条规则会自动放弃，不需要额外表达“没现金就放弃”这层意思）、exitAll(全部清仓，value必须是null)。“调仓到/仓位调整为/仓位设为”后面跟一个具体百分比数字，是 targetPercent；“加仓/减仓/增加/减少仓位”后面跟一个百分比数字，是 addPercent/reducePercent，不要混淆——前者是绝对值，后者是相对当前仓位的增量。",
